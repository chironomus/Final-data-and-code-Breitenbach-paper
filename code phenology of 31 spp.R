#starting from the original excel files, only modification is to remove species with  low abundance manually, #since it is faster in excel, all other manipulations 
...
#abundnce of 31 spp analyzed for phenology
#combine 7 most declining spp into data set
sp7B=rbind(Bro,Bve,Pro_int,Aga,Leu_nig,Tin_ros,Cha)
sp7B=as.data.frame(sp7B)
sp7_1=aggregate(sp7B[,5], list(sp7B$Year),sum)#sum of all selected for phenology taxa
#total abundance siteB
totab= read.table("totab.txt",sep=",",header = TRUE) #reading in river data

totab=totab[,1:2]

#total and 31 sp abund combined
sp7_1$total_abundance=totab$x
sp7_1$abundance_7_species=sp7_1$x
plot(sp7_1$total_abundance~sp7_1$Group.1,col="red",pch=19, ylim=c(0,40000),xlab="year",ylab="individuals")
lines(sp7_1$total_abundance~sp7_1$Group.1,col="red")
points(sp7_1$abundance_7_species~sp7_1$Group.1,col="black",pch=19)
lines(sp7_1$abundance_7_species~sp7_1$Group.1,col="black")
